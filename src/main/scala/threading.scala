...
class MyThread(startIdx: Int, rd: Random, idsMagasin: Array[String]) extends Thread {
  ...
  override def run()
  {
    ...
    val path = "hdfs://127.0.0.1:/tmp/v2/part_"+ this.startIdx +".parquet"
    println(path)
    ...
    for (j <- this.startIdx*100000 until 1000000*(this.startIdx+1)){
      ...
      for( r <- 0 until repeatTransaction){
        ...
        val transaction = Transaction(timeStamp,productId.toString,(j + 1).toString,this.idsMagasin(indexMagasin),qte.toString)
...
object threading
{
  ...
  def main(args: Array[String])
  {
    ...
    for (k <- 0 until 100)
    {
      var th = new MyThread(k, rd, idsMagasin)
      th.setName(k.toString())
      th.start()
    }
  }
}
